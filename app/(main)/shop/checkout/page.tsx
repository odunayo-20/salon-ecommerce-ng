...
export default function CheckoutPage() {
  ...
  const effectiveRedeemed = Math.min(
    pointsRedeemed,
    maxRedeemable,
    loyaltyBalance
  );
  const grandTotal = Math.max(
    afterDiscount + shipping - effectiveRedeemed,
    0
  );
  ...
  const [paymentMethod, setPaymentMethod] = useState<
    "card" | "bank_transfer" | "pay_on_delivery"
  >("card");
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [error, setError] = useState("");

  // Coupon input state (on-checkout)
  ...
  const handleApplyCoupon = async () => {
    ...
    try {
      const res = await fetch("/api/coupons/validate", {
        ...
        body: JSON.stringify({ code: couponInput.trim(), subtotal: total, type: "PRODUCTS" }),
      });
      ...
      applyCoupon({
        ...
        description: data.coupon.description || `${data.coupon.value}${data.coupon.type === "PERCENTAGE" ? "%" : " off"}`,
      });
      setPointsInput("");
      setPointsRedeemed(0);
      setCouponInput("");
    ...
    return (
      <div className="min-h-screen bg-cream">
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 py-20 text-center">
          ...
          <Link href="/auth/signin?callbackUrl=/shop/checkout">
            <Button className="bg-charcoal text-white hover:bg-charcoal-light rounded-full px-8 text-xs font-semibold tracking-wider uppercase">
              Sign In
            </Button>
          </Link>
  ...
  const handlePlaceOrder = async () => {
    ...
    try {
      ...
      if (payData.provider === "stripe" && payData.clientSecret) {
        // Stripe: show PaymentElement inline
        setStripeClientSecret(payData.clientSecret);
        setStripePaymentId(payData.paymentId);
  ...
  const handleStripeSuccess = () => {
    clearCart();
    router.push(
      `/shop/order/success?orderId=${stripeOrderId}`
    );
  };
                ...
                {[
                  {
                    value: "card" as const,
                    label: "Pay with Card",
                    desc: "Debit or credit card via Stripe",
                    icon: "💳",
                  },
                  {
                    value: "bank_transfer" as const,
                    label: "Bank Transfer",
                    desc: "Pay via bank transfer via Paystack",
                    icon: "🏦",
                  },
                  {
          ...
          <div>
            <div className="bg-white border border-border rounded-xl p-6 sticky top-24">
              ...
              <div className="border-t border-border pt-3 space-y-2 text-sm">
                ...
                {coupon ? (
                  <div className="bg-green-50 border border-green-200 rounded-lg p-3 mt-2">
                    <div className="flex items-center justify-between">
                      ...
                      <button onClick={() => { removeCoupon(); }} className="text-muted-foreground hover:text-red-500 transition-colors p-1">
                  ...
                  <div className="mt-2">
                    <div className="flex items-center gap-2">
                      <Input
                        ...
                        className="h-8 text-xs bg-cream border-border rounded-full flex-1"
                      ...
                      <Button
                        ...
                        size="sm"
                        variant="outline"
                        className="h-8 text-[10px] rounded-full border-border text-xs shrink-0"
                        onClick={handleApplyCoupon}
                        disabled={couponLoading || !couponInput.trim()}
                      >
                        {couponLoading ? <Loader2 className="h-3 w-3 animate-spin" /> : <><Tag className="h-3 w-3 mr-1" />Apply</>}
                      </Button>
                    </div>
                ...
                {!loyaltyLoading && loyaltyBalance > 0 && !coupon && (
                  <div className="bg-gold/5 border border-gold/20 rounded-lg p-3 mt-2">
                    ...
                    {effectiveRedeemed > 0 && (
                      <div className="flex items-center gap-1 mt-2 text-[10px] text-green-600">
                        ...
                        Redeeming {effectiveRedeemed.toLocaleString()} pts
                        (-₦{effectiveRedeemed.toLocaleString()})
                      </div>
                    )}
                    <p className="text-[10px] text-muted-foreground mt-1">
                      Max 50% of subtotal. 1 pt = ₦1 off.
                    </p>
                  </div>
                )}
              ...
              <div className="flex items-center gap-2 mt-4 text-[10px] text-muted-foreground">
                ...
                {paymentMethod === "card" ? "Stripe" : "Paystack"}
              </div>
              ...
              {/* Hide Place Order button when Stripe form is visible */}
              {!stripeClientSecret && (
                <Button
                  onClick={handlePlaceOrder}
                  disabled={isSubmitting}
                  className="w-full mt-4 bg-gold text-white hover:bg-gold-dark rounded-full py-6 text-xs font-semibold tracking-wider uppercase disabled:opacity-50"
                >
                  {isSubmitting ? (
                    <>
