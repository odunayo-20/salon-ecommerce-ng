...
import { Loader2, Search, AlertTriangle, Package, ArrowUpDown, Upload, X } from "lucide-react";
  ...
  return (
    <div className="space-y-5">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          <h1 className="font-heading text-xl sm:text-2xl font-bold text-charcoal">Inventory</h1>
        ...
        <div className="flex items-center gap-2">
          {tab === "stock" && (
            <>
              <Button onClick={() => setBulkMode(!bulkMode)} variant={bulkMode ? "default" : "outline"} size="sm" className={cn("rounded-full text-xs min-h-[44px]", bulkMode && "bg-gold text-white")}>
                <Upload className="h-3 w-3 mr-1" />Bulk
              </Button>
              {bulkMode && Object.keys(bulkChanges).length > 0 && (
                <Button onClick={handleBulkSave} disabled={bulkUpdate.isPending} size="sm" className="bg-gold text-white hover:bg-gold-dark rounded-full text-xs min-h-[44px]">
      ...
      <div className="flex gap-1 bg-white border border-border rounded-lg p-1">
        {([["stock", "Stock"], ["movements", "Movements"]] as [Tab, string][]).map(([t, label]) => (
          <button key={t} onClick={() => setTab(t)} className={cn("flex-1 py-2.5 text-sm font-medium rounded-md transition-colors min-h-[44px]", tab === t ? "bg-charcoal text-white" : "text-muted-foreground hover:text-charcoal")}>{label}</button>
        ))}
      </div>
      ...
      {tab === "stock" && (
        <>
          <div className="relative">
            ...
            <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search products..." className="w-full bg-white border border-border rounded-lg pl-10 pr-4 py-2.5 text-sm focus:outline-none focus:border-gold min-h-[44px]" />
          </div>

          <Button onClick={() => setLowStockOnly(!lowStockOnly)} variant={lowStockOnly ? "default" : "outline"} size="sm" className={cn("rounded-full text-xs min-h-[44px] w-full", lowStockOnly && "bg-amber-500 text-white hover:bg-amber-600")}>
                ...
                {data.products.map((p) => (
                  <div key={p.id} className="bg-white border border-border rounded-xl p-4">
                    <div className="flex items-start gap-3 mb-3">
                      <div className="h-12 w-12 bg-cream rounded-lg overflow-hidden shrink-0 flex items-center justify-center">
                        {p.image && p.image.trim() ? <Image src={p.image} alt={p.name} width={48} height={48} className="h-full w-full object-cover" /> : <Package className="h-5 w-5 text-muted-foreground" />}
                      </div>
                      ...
                      {p.isOutOfStock ? <span className="text-[10px] font-semibold bg-red-50 text-red-600 px-2 py-0.5 rounded-full shrink-0">Out</span>
                        : p.isLowStock ? <span className="text-[10px] font-semibold bg-amber-50 text-amber-600 px-2 py-0.5 rounded-full shrink-0">Low</span>
                        : <span className="text-[10px] font-semibold bg-emerald-50 text-emerald-700 px-2 py-0.5 rounded-full shrink-0">OK</span>}
                    </div>
                    <div className="flex items-center justify-between pt-2 border-t border-border/50">
                      <div className="flex items-center gap-2">
                        ...
                        {bulkMode ? (
                          ...
                          <span className={cn("text-sm font-bold", p.isOutOfStock ? "text-red-600" : p.isLowStock ? "text-amber-600" : "text-charcoal")}>{p.stock}</span>
                        )}
                      </div>
                      {!bulkMode && (
                        <Button onClick={() => setAdjustDialog({ productId: p.id, name: p.name, currentStock: p.stock })} variant="outline" size="sm" className="rounded-full text-xs min-h-[44px]">
                          ...
                          <td className="px-4 py-3">
                            <div className="flex items-center gap-3">
                              <div className="h-10 w-10 bg-cream rounded-lg overflow-hidden shrink-0 flex items-center justify-center">
                                {p.image && p.image.trim() ? <Image src={p.image} alt={p.name} width={40} height={40} className="h-full w-full object-cover" /> : <Package className="h-4 w-4 text-muted-foreground" />}
                              </div>
                ...
                {data.recentMovements.map((m) => (
                  <div key={m.id} className="bg-white border border-border rounded-xl p-4">
                    <div className="flex items-start justify-between mb-2">
                      <div>
                        <p className="text-sm font-medium text-charcoal">{m.productName}</p>
                        {m.variantName && <p className="text-xs text-muted-foreground">({m.variantName})</p>}
                      </div>
                      <span className={cn("text-[10px] font-semibold px-2 py-0.5 rounded-full shrink-0", typeColors[m.type] || "bg-gray-50 text-gray-600")}>{m.type}</span>
                    </div>
                    <div className="grid grid-cols-3 gap-2 text-xs mb-2">
                      <div><span className="text-muted-foreground">Qty</span><p className={cn("font-medium", m.quantity > 0 ? "text-emerald-600" : "text-red-500")}>{m.quantity > 0 ? "+" : ""}{m.quantity}</p></div>
                      <div><span className="text-muted-foreground">Before</span><p className="text-charcoal">{m.previousQty}</p></div>
                      <div><span className="text-muted-foreground">After</span><p className="font-medium text-charcoal">{m.newQty}</p></div>
                    </div>
                    <div className="flex items-center justify-between text-xs text-muted-foreground pt-2 border-t border-border/50">
                      <span className="truncate max-w-[60%]">{m.note || "No note"}</span>
                      <span className="whitespace-nowrap">{new Date(m.createdAt).toLocaleDateString("en-NG", { month: "short", day: "numeric", hour: "2-digit", minute: "2-digit" })}</span>
                    </div>
                  </div>
                ))}
              </div>
      ...
      {adjustDialog && (
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center sm:p-4">
          ...
          <div className="relative bg-white rounded-t-2xl sm:rounded-2xl shadow-2xl w-full sm:max-w-sm p-6 space-y-4">
            <div className="flex items-center justify-between">
              <h3 className="font-heading text-lg font-semibold text-charcoal">Adjust Stock</h3>
              <button onClick={() => setAdjustDialog(null)} className="text-muted-foreground hover:text-charcoal min-h-[44px] min-w-[44px] flex items-center justify-center"><X className="h-5 w-5" /></button>
            </div>
            ...
            <div>
              ...
              <select value={adjustType} onChange={(e) => setAdjustType(e.target.value)} className="mt-1 w-full border border-border rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:border-gold min-h-[44px]">
            ...
            <div>
              ...
              <input type="number" min="0" value={adjustQty} onChange={(e) => setAdjustQty(parseInt(e.target.value) || 0)} className="mt-1 w-full border border-border rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:border-gold min-h-[44px]" />
            ...
            <div>
              ...
              <input value={adjustNote} onChange={(e) => setAdjustNote(e.target.value)} placeholder="Reason for adjustment" className="mt-1 w-full border border-border rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:border-gold min-h-[44px]" />
            </div>
            <div className="flex gap-3">
              <Button variant="outline" onClick={() => setAdjustDialog(null)} className="flex-1 rounded-full text-xs min-h-[44px]">Cancel</Button>
              <Button onClick={handleAdjust} disabled={adjustQty === 0 || adjustStock.isPending} className="flex-1 bg-gold text-white hover:bg-gold-dark rounded-full text-xs font-semibold min-h-[44px]">
