    ...
    <div className="space-y-6">
      ...
      {successMsg && (
        <div className="bg-green-50 border border-green-200 text-green-700 text-sm rounded-lg px-4 py-3 flex items-center justify-between">
          ...
          <button onClick={() => setSuccessMsg("")} className="p-2.5 min-h-[44px] min-w-[44px] flex items-center justify-center rounded-lg hover:bg-green-100 transition-colors">
            <X className="h-4 w-4" />
          </button>
        </div>
      )}
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        ...
        <div className="flex gap-1 bg-white border border-border rounded-lg p-1">
          {(["all", "pending", "approved"] as const).map((f) => (
            <button key={f} onClick={() => setFilter(f)} className={cn("px-3 py-2.5 min-h-[44px] rounded-md text-xs font-medium transition-all capitalize", filter === f ? "bg-charcoal text-white" : "text-muted-foreground hover:text-charcoal")}>
              {f}
      ...
      {isLoading ? (
        ...
        <>
          <div className="sm:hidden space-y-3">
            {reviews.map((r) => (
              <div key={r.id} className="bg-white border border-border rounded-2xl p-4 space-y-3">
                <div className="flex items-center gap-3">
                  <div className="h-10 w-10 rounded-full bg-cream flex items-center justify-center text-sm font-bold text-charcoal shrink-0 overflow-hidden">
                    {r.customerImage ? <img src={r.customerImage} alt="" className="h-full w-full object-cover" /> : (r.customer || "A").charAt(0)}
                  </div>
                  <div className="min-w-0 flex-1">
                    <p className="text-sm font-medium text-charcoal truncate">{r.customer || "Anonymous"}</p>
                    <div className="flex gap-0.5 mt-0.5">
                      {Array.from({ length: 5 }).map((_, i) => (
                        <Star key={i} className={cn("h-3 w-3", i < r.rating ? "fill-gold text-gold" : "text-gray-200")} />
                      ))}
                    </div>
                  </div>
                  <span className={cn("inline-flex px-2.5 py-0.5 rounded-full text-[10px] font-semibold uppercase tracking-wider shrink-0", r.isApproved ? "bg-emerald-50 text-emerald-700" : "bg-amber-50 text-amber-700")}>
                    {r.isApproved ? "Approved" : "Pending"}
                  </span>
                </div>
                {(r.productName || r.serviceName) && (
                  <span className="inline-block text-[10px] bg-cream px-2.5 py-1 rounded-full text-muted-foreground font-medium">
                    {r.productName || r.serviceName}
                  </span>
                )}
                ...
                <div className="flex items-center gap-2 pt-1 border-t border-border/50">
                  {!r.isApproved ? (
                    <button onClick={() => toggleApproval(r.id, true)} disabled={updatingId === r.id} className="p-2.5 min-h-[44px] min-w-[44px] rounded-lg text-muted-foreground hover:text-emerald-600 hover:bg-emerald-50 transition-colors flex items-center justify-center" title="Approve">
                      <CheckCircle2 className="h-4 w-4" />
                    </button>
                  ) : (
                    <button onClick={() => toggleApproval(r.id, false)} disabled={updatingId === r.id} className="p-2.5 min-h-[44px] min-w-[44px] rounded-lg text-muted-foreground hover:text-amber-600 hover:bg-amber-50 transition-colors flex items-center justify-center" title="Unapprove">
                      <XCircle className="h-4 w-4" />
                    </button>
                  )}
                  <button onClick={() => toggleFeatured(r.id, !r.isFeatured)} disabled={updatingId === r.id} className={cn("p-2.5 min-h-[44px] min-w-[44px] rounded-lg transition-colors flex items-center justify-center", r.isFeatured ? "text-gold hover:bg-gold/10" : "text-muted-foreground hover:text-gold hover:bg-gold/10")} title={r.isFeatured ? "Unfeature" : "Feature"}>
                    {r.isFeatured ? <Star className="h-4 w-4 fill-gold" /> : <StarOff className="h-4 w-4" />}
                  </button>
                  <button onClick={() => handleDelete(r)} disabled={updatingId === r.id} className="p-2.5 min-h-[44px] min-w-[44px] rounded-lg text-muted-foreground hover:text-red-500 hover:bg-red-50 transition-colors flex items-center justify-center" title="Delete">
